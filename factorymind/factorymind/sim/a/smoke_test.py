...
def _run_mock() -> None:
    ...
    env_empty = MockCellEnv(num_robots=2, seed=0, scenario="empty_bin")
    final_empty = run_oracle_episode(env_empty)
    assert final_empty["done"], f"Mock empty_bin failed: {final_empty}"
    assert "empty_bin_diagnosed" in final_empty["events"], f"Mock empty_bin missing diagnosis: {final_empty}"
    print(f"OK [mock empty_bin] — diagnosed in {final_empty['step']} steps")
...
def _run_mujoco() -> None:
    ...
    env_empty = create_cell_env(SimConfig(backend="mujoco", scenario="empty_bin", default_seed=0))
    final_empty = run_oracle_episode(env_empty)
    assert final_empty["done"], f"MuJoCo empty_bin failed: {final_empty}"
    assert "empty_bin_diagnosed" in final_empty["events"], f"MuJoCo empty_bin missing diagnosis: {final_empty}"
    print(f"OK [mujoco empty_bin] — diagnosed in {final_empty['step']} steps")
